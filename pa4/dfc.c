...
// | | | | |_    | |   | | |/ _ \ '_ \| __|
// | |_| |  _|   | |___| | |  __/ | | | |_ 
// |____/|_|      \____|_|_|\___|_| |_|\__|
//
// Distributed file client : PA4
// Ryan Taylor
...
void createSocket (int dfsIdx, int port) {
    ...
    if (setsockopt (sockfd[dfsIdx], SOL_SOCKET, SO_REUSEADDR, (char*) &timeout, sizeof (timeout)) < 0) {
...
tryagain:
    if (connect (sockfd[dfsIdx], (struct sockaddr*) &serveraddr[dfsIdx], sizeof (struct sockaddr_in)) != 0) {
        if (timedout == 1) {
            printf ("\nError connecting to DFS%d, exiting", dfsIdx + 1);
            exit (1);
        }
        else {
            sleep (1);
